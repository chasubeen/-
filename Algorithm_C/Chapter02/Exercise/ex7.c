...
int card_conv(unsigned x, int n, char d[]){
	int i;
	char dchar[] = "0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZ"; // 진수 변환 값(10부터는 A ~ )
	...
	if (x == 0)										// 0이면 
		...
		while (x) { // x를 나눌 수 있을 때까지
			printf("%2d | %7d ... %c\n", n, x, dchar[x % n]);
			printf("   + ---------- \n");
			d[digits++] = dchar[x % n];				 // n으로 나눈 나머지를 저장해 놓기
			x /= n; // x 갱신
		}
		printf("%12d\n", x);
	}

	/* d[0] ~ d[digits -1]의 값을 역순으로 정렬 */
...
void main(){
	...
	do {
		...
		do {
			...
			scanf("%d", &cd);
		} while (cd < 2 || cd > 36); // 입력받는 수의 범위 제한
